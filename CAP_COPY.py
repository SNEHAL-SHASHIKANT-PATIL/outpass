...
def web_now():
    """
    Returns current date & time as per IST (Asia/Kolkata)
    Used instead of datetime.now()
    """
...
def web_date_str():
    ...
    """
    return web_now().strftime("%Y-%m-%d")

def web_time_str():
    """
    Returns current time in HH:MM format
    """
    return web_now().strftime("%H:%M")
...
def update_status_email(req_id, status):
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        Aaction_time = web_now().strftime("%d-%m-%Y %I:%M %p")
...
def update_status():
    ...
    # ✅ WEB TIME (IST) – NOT SYSTEM TIME
    action_time = web_now().strftime("%d-%m-%Y %I:%M %p")
    ...
    cursor.execute("""
        ...
        WHERE Enrollment_No = ?
    """, (status, action_time, enrollment))

    conn.commit()
    conn.close()

    return jsonify({"time": action_time})
...
def warden_summary():
    ...
    for r in rows:
        ...
        <a href="{{ url_for('export_logs') }}" class="top-right-btn-export">
    Export Logs
</a>
