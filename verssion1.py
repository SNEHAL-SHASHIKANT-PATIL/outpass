...
def warden_summary():
    ...
    if enroll_list:
        ...
        cursor.execute(f"""
            SELECT id, Enrollment_No, From_Date, To_Date,
                   Out_Time, In_Time,
    ...
    for r in rows:
        (
            req_id, enroll, from_date, to_date,
            out_time, in_time,
            reason, hostel_name, address, status
        ) = r
        ...
        <div id="card_{req_id}"
             style="display:none;margin-top:10px;width:300px;
                    background:#f0f7ff;border-radius:15px;
            ...
            <small><b>From Date:</b> {from_date}</small><br>
            <small><b>To Date:</b> {to_date}</small><br>
            <small><b>Out Time:</b> {out_time if out_time else '-'}</small><br>
            <small><b>In Time:</b> {in_time if in_time else '-'}</small><br>

            <small><b>Reason:</b> {reason}</small><br>
